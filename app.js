...
function getDive(query, callback) {
  const tasteDive_URL =
    ...
    query +
    "&type=shows&info=1&limit=20&k=284615-BingeOn-6N8G8Z98";
  $.ajax({
    url: tasteDive_URL,
    q: "${query} in:name",
    type: "shows",
  ...
  win
    .resize(function() {
      const win_w = win.width(),
        win_h = win.height(),
        $bg = $("#bg");
      const available = [1024, 1280, 1366, 1400, 1680, 1920, 2560, 3840, 4860];
      const current = $bg.attr("src").match(/([0-9]+)/) ? RegExp.$1 : null;
      if (
        !current ||
        (current < win_w && current < available[available.length - 1])
      ) {
